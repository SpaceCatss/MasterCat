...
var arr,asc=true, o = argument[0], p = ds_priority_create();
if (argument_count>2) {asc = argument[2];}

for (var arr = instance_select_array(argument[0]), i=0, s=array_length_1d(arr); i<s; i++) {
    var inst=arr[i], value = 0;
    with(inst) {
        value = script_execute(argument[1]);
    }
